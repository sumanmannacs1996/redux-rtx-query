...
import { useAddTaskMutation, useDeleteTaskMutation, useGetTasksQuery, useUpdateTaskMutation } from "./featurs/apiSlice";
import { useAddCustomQueryTaskMutation, useDeleteCustomQueryTaskMutation, useGetCustomQueryTasksQuery, useUpdateCustomQueryTaskMutation } from "./featurs/apiSliceCustomQuery";

export default function Home() {
  const [newTask, setNewTask] = useState("");

  // const { data: tasksList, isFetching, isLoading, isSuccess, isError, error, refetch } = useGetTasksQuery();
  const { data: tasksList, isFetching, isLoading, isSuccess, isError, error, refetch } = useGetCustomQueryTasksQuery();

  // const [addTask] = useAddTaskMutation();
  // const [updateTask] = useUpdateTaskMutation();
  // const [deleteTask] = useDeleteTaskMutation();
  const [addTask] = useAddCustomQueryTaskMutation();
  const [updateTask] = useUpdateCustomQueryTaskMutation();
  const [deleteTask] = useDeleteCustomQueryTaskMutation();
      ...
      <div className="task-app w-full max-w-md rounded-lg bg-gray-800 px-6 pb-2 pt-6 text-gray-200 shadow-lg">
        ...
        <form
          onSubmit={(e) => {
            ...
            addTask(task);
            // refetch();   // not require as we used tags
            setNewTask("");
          }}
